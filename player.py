...
class Player:
    ...
    def affiche_player(self):
        e = 3
        self.player = self.canvas.create_image(self.px*self.var.SIZE+e,
                                               self.py*self.var.SIZE+e,
                                               anchor=NW,
                                               image=self.mcgyver[0])
        self.canvas.tag_raise(self.player)

    def move_player(self, xi, yi, level):
        x = self.px + xi
        y = self.py + yi
        # Floor
        if level[y][x] == 0:
        ...
        e = 3
        img = self.canvas.create_image(self.px*self.var.SIZE+e,
                                       self.py*self.var.SIZE+e,
                                       anchor=NW,
                                       image=self.mcgyver[2])
        self.canvas.coords(self.player,
                           self.px*self.var.SIZE+e,
                           self.py*self.var.SIZE+e)
        self.canvas.tag_raise(self.player)
